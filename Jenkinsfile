pipeline {
  ...
  stages {
    ...
    stage('Triger CD Pipeline') {
      steps {
        script {
          sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'https://jenkins.devpro.life/job//job/gitops-register-app-cd/build?token=gitops-token'"
        }
      }
    }
  }
