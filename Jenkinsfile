  ...
  stages {
   ...
   stage("Build & Push Docker Image") {
    ...
    stage("Trigger CD Pipeline") {
      steps {
        script {
          sh "curl -v -k --user avinash:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'https://jenkins.devpro.life/job/gitips-register-app-cd/buildWithParameters?token=gitops-token'"
        }
      }
    }
  }
